    ...
    def generate_reply(
        ...
        if chat_mode:
            ...
            if candidates:
                ...
                kb_block = (
                    "Факты из базы знаний (если подходят — используй):\n" + "\n".join(facts)
                    + "\n\nЕсли фактов нет или они не подходят — ответь свободно, по-дружески. НЕ предлагай передать модератору."
                )
            else:
                kb_block = "В базе нет фактов по этому запросу. Ответь свободно, по-дружески. НЕ предлагай передать модератору."
            system = (
                "Ты Buddy — друг в PravoTech. Пользователь хочет поболтать. "
                "Отвечай свободно, по-человечески. «Как дела», «расскажи о компании», «что умеешь» — отвечай тепло. "
                "Можешь использовать общие знания о компании (PravoTech). НЕ предлагай передать вопрос модератору — это только для рабочих процедур.\n\n"
            )
        else:
            kb_block = ""
            if candidates:
                ...
                kb_block = (
                    ...
                    + "\n\nОтвечай ТОЛЬКО если факт напрямую отвечает на вопрос. "
                    "Если нет — скажи честно и предложи передать модератору."
                )
            else:
                kb_block = (
                    "В базе нет релевантных фактов. Скажи честно и предложи передать вопрос модератору."
                )
            system = (
