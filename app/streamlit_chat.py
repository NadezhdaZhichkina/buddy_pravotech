...
class StreamlitChatService:
    ...
    def generate_reply(
        ...
        """
        Генеративный режим. chat_mode=True — поболтать, отвечать свободно, не эскалировать.
        chat_mode=False — рабочие вопросы: из БЗ или эскалация.
        """
        if not self.llm_enabled:
            if skip_retrieval:
        ...
        if chat_mode:
            ...
            if candidates:
                ...
                kb_block = (
                    "Факты (если подходят — используй):\n" + "\n".join(facts)
                    + "\n\nИначе — ответь по-человечески. Не предлагай модератора."
                )
            else:
                kb_block = "Фактов нет. Отвечай как живой человек. Не предлагай модератора."
            system = (
                "Ты Buddy — друг. Общаешься как живой человек: тепло, естественно, без шаблонов. "
                "«Как дела», «расскажи о компании» — отвечай по-дружески. Можешь делиться общим о PravoTech. "
                "НЕ предлагай передать модератору.\n\n"
            )
        else:
            kb_block = ""
            if candidates:
                ...
                kb_block = (
                    "Факты:\n" + "\n".join(facts)
                    + "\n\nЕсли факт отвечает на вопрос — ответь своим словами, по-человечески. "
                    "Если нет — честно скажи и предложи передать модератору."
                )
            else:
                kb_block = "Фактов нет. Честно скажи и предложи передать модератору."
            system = (
                "Ты Buddy — друг. Общайся как живой человек: тепло, естественно. "
                "«Что делать дальше» — направляй к менеджеру по адаптации. "
                "На вопросы про компанию — отвечай из базы. Нет в базе — честно скажи.\n\n"
            )
        ...
        system = system + f"Профиль: роль={role}, круг={circle}. Сделано: {', '.join(done) or 'ничего'}. {nt}\n\n{kb_block}"
    ...
    def _fallback_answer(self, candidates: list[KnowledgeItem]) -> str:
        if not candidates:
            return (
                "Честно говоря, по этому запросу в базе пока мало данных. "
                "Можешь уточнить вопрос или передам модератору — подскажут точнее."
            )
        if len(candidates) == 1:
            return f"{candidates[0].answer}"
        return (
            f"{candidates[0].answer}\n\n"
            f"Ещё: {candidates[1].answer}"
        )
