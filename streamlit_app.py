"""
Buddy — друг новичка. Streamlit-интерфейс для тестирования.
...
# --- Константы (все в одном месте) ---
QUESTION_WORDS = (
    "как", "что", "где", "когда", "зачем", "почему", "кто", "какой", "какая", "какие",
    "можно", "нужно", "подскажи",
)
DONE_WORDS = (
    "сделал", "сделала", "готов", "готово", "готова", "уже", "отправил", "отправила",
    "настроил", "настроила", "подписался", "подписалась", "проверил", "проверила",
)
PROBLEM_WORDS = ("проблем", "ошибка", "не работает", "не получается", "не могу", "нет доступа", "не заходит", "сломал", "сломалось")
NO_PROBLEM_WORDS = ("нет проблем", "все ок", "всё ок", "все хорошо", "всё хорошо", "нормально", "в порядке")
NEXT_STEP_WORDS = ("что дальше", "что делать дальше", "что мне делать", "что делать", "дальше")
CIRCLE_ALIASES = {
    "маркет": "маркетинг", "marketing": "маркетинг", "sales": "sales", "продаж": "sales",
    "product": "product", "продукт": "product", "docs": "docs", "legal": "legal",
    "new business": "new business", "nb": "new business", "projects": "projects",
    "инфра": "инфраструктура и ит", "it": "инфраструктура и ит", "hr": "hr",
    "client care": "client care", "work": "work",
}
CIRCLE_INTERACTIONS = {
...
def _looks_like_how_are_you(text: str) -> bool:
    """«Как дела?», «как ты?» — даём прямой дружеский ответ без LLM."""
    t = (text or "").strip().lower().rstrip("?!. ")
    if not t or len(t) > 20:
        return False
    return t in ("как дела", "как ты", "как сам", "как самa") or t.startswith("как дела") or t.startswith("как ты ")


def _is_how_are_you(text: str) -> bool:
    """«Как дела?», «как ты» — короткое приветствие, ответить сразу без LLM."""
    t = (text or "").strip().lower().rstrip("?!.")
    if not t or len(t) > 15:
        return False
    return t in ("как дела", "как ты", "как сама", "как сам", "how are you")


def _looks_like_chat(text: str) -> bool:
    """Поболтать, не рабочий вопрос — отвечать свободно, не эскалировать."""
    t = (text or "").strip().lower()
    if not t or len(t) > 80:
        return False
    chat_markers = (
        "как дела", "как ты", "что умеешь", "расскажи о себе", "обучаешься", "обучаешься ли",
        "поболтать", "что интересного", "расскажи что-нибудь", "расскажи интересное",
        "чем заняться", "чем заняться еще", "что делать", "расскажи", "ну расскажи",
        "расскажи о компании", "что интересного расскажешь",
        "как тебя", "кто ты такой", "что ты такое", "ты кто", "ты что",
    )
    if any(m in t for m in chat_markers):
        return True
    # Короткие общие: "расскажи", "ну расскажи", "а что ты"
    if t in ("расскажи", "ну расскажи", "а что", "и что", "давай"):
        return True
    return False


def _looks_like_work_question(text: str) -> bool:
    """Вопрос про работу: процедуры, аббревиатуры, регламенты — искать в БЗ, при отсутствии эскалировать."""
    t = (text or "").strip().lower()
    if not t:
        return False
    work_markers = (
        "что такое ", "как оформить", "как подать", "где находится", "где посмотреть",
        "какой ", "какая ", "какие ", "кто ответственный", "куда обращаться",
        "процесс", "регламент", "доступ", "оформлен", "заявк", "отпуск", "больничн",
        "кэдо", "ипр", "okr", "focus", "mchat", "vpn", "hr.", "crm",
    )
    if any(m in t for m in work_markers):
        return True
    if _looks_like_term_or_abbreviation_query(text):
        return True
    if "?" in t and any(w in t for w in ("как", "что", "где", "когда", "зачем", "почему", "кто")):
        return True
    return False


def _looks_like_real_question(text: str) -> bool:
    """Сообщение — реальный вопрос, имеет смысл искать в базе. Иначе — дружеский ответ без поиска."""
    t = (text or "").strip()
    if not t or len(t) < 3:
        return False
    low = t.lower()
    if "?" in t:
        return True
    # Вопросные слова в начале
    if any(low.startswith(w + " ") or low == w for w in QUESTION_WORDS):
        return True
    # Запрос про термин/аббревиатуру (что такое КБ, расшифровка ИПР)
    if _looks_like_term_or_abbreviation_query(t):
        return True
    # Длинное сообщение (>20 символов) — вероятно вопрос
    if len(t) > 20:
        return True
    return False
...
def _recent_history(n: int = 10) -> list[dict]:
    """Последние n сообщений в формате для generate_reply / _apply_informative_user_message."""
    return [
...
def _starter_plan(role: str, circle: str) -> str:
    ...
    return (
        ...
        "Я Buddy — просто друг. Подскажу по культуре, чатам, знакомствам. "
        "По вопросам что делать дальше — менеджер по адаптации.\n\n"
        "Напиши, что сделала — отмечу. Пиши что угодно: «пост отправила», «сервисы настроила» и т.д. "
...
def _small_talk_reply(next_task: dict | None) -> str:
    ...
    if next_task:
        return (
            f"{base}\n\n"
            f"По плану дальше: **{next_task['title']}** — {next_task['hint']}\n\n"
            "По вопросам что делать — менеджер по адаптации. А я могу рассказать о компании, каналах, ритуалах."
        )
    return (
        f"{base}\n\n"
        "Если хочешь, могу:\n"
...
def _build_informative_ack(profile: dict, info: dict, keep_pending_offer: bool = False) -> str:
    ...
    if next_task:
        parts.append(f"Кстати, по плану дальше: **{next_task['title']}** — {next_task['hint']}\n\nПо вопросам что делать — менеджер по адаптации.")
    else:
...
def _next_step_response(profile: dict, next_task: dict | None) -> str:
    ...
    if repeats >= 1:
        return (
            ...
            f"По плану: **{next_task['title']}** — {next_task['hint']}\n\n"
            "По вопросам что делать — менеджер по адаптации. Могу подсказать что-нибудь или ответить на вопрос."
        )
    return (
        f"По плану дальше: **{next_task['title']}** — {next_task['hint']}\n\n"
        "По вопросам что делать — менеджер по адаптации."
...
st.title("🤖 Buddy")
st.caption("Просто друг — подскажу что знаю, поболтаем.")
...
if not is_moderator:
    ...
    if user_updates:
        # Вопрос уже обработан — убираем «готовый к отправке», чтобы не спрашивать снова
        st.session_state.pending_ticket_offer_by_user[st.session_state.chat_username] = None
...
def _recent_messages_have_moderator_answer(messages: list[dict]) -> bool:
    """Есть ли в последних сообщениях ответ модератора — тогда pending уже не актуален."""
    if not messages:
        return False
    for m in messages[-6:]:
        if m.get("role") == "assistant":
            c = (m.get("content") or "").lower()
            if "модератор обработал" in c or "модератор обработал вопрос" in c:
                return True
    return False
...
if show_user_panel:
    ...
    # Если модератор уже ответил — pending не актуален
    if pending_offer and _recent_messages_have_moderator_answer(st.session_state.messages):
            ...
            try:
                ...
                if pending_offer:
                    if _is_yes_reply(prompt):
                        ...
                        info = _apply_informative_user_message(profile, prompt, _recent_history())
                        if info["informative"]:
                            role = profile.get("role")
                            circle = profile.get("circle")
                            ...
                            # «как дела» и т.п. — даём живой ответ, а не только напоминание о тикете
                            if any(m in (prompt or "").lower() for m in ("как дела", "как ты")):
                                response = (
                                    "Отлично, спасибо! Рада общаться 🙂 "
                                    "Чем могу помочь?\n\n"
                                    "Кстати, по предыдущему вопросу — готов передать модератору, если хочешь. "
                ...
                if not handled_ticket_offer and _is_direct_moderator_request(prompt):
                    ticket_question = _extract_question_from_history_for_ticket(_recent_history(), prompt)
                    if ticket_question:
                        created_ticket_id = service.create_moderation_ticket(
                ...
                if not handled_ticket_offer:
                    ...
                    if _looks_like_complaint(prompt):
                    ...
                    elif not _looks_like_real_question(prompt):
                        response = (
                            "Чем могу помочь? Спроси что угодно — про процессы, аббревиатуры, каналы. "
                            "Или напиши, что сделала по онбордингу 🙌"
                        )
                        ...
                        # Не вопрос — дружеский ответ без поиска в базе (иначе тянет случайные факты)
                        response = (
                            "Чем могу помочь? Можешь спросить что угодно — про процессы, аббревиатуры, каналы. "
                            "Или напиши, что уже сделала по онбордингу 👍"
                        )
                    elif any(m in (prompt or "").lower() for m in ("как дела", "как ты")) and len((prompt or "").strip()) <= 25:
                        # «как дела?» — быстрый ответ без LLM (надёжно, без таймаутов)
                        response = "Отлично, спасибо! Рада общаться 🙂 Чем могу помочь?"
                    else:
                        is_chat = _looks_like_chat(prompt)
                    ...
                    # Заменять на тикет только для рабочих вопросов. Поболтать — не эскалировать.
                    ...
                    if (
                        ...
                        and not _looks_like_chat(prompt)
                        and (_looks_like_work_question(prompt) or _looks_like_term_or_abbreviation_query(prompt) or explicit_escalation)
                        and (
                            explicit_escalation
